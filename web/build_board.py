...
<script>
 ...
 function render(){
  ...
  let rows=[];
  if(P==='oos'){
    DATA.filter(s=>s.oos).forEach(s=>{(L==='all'?['1x','2x','3x']:[L]).forEach(lv=>{const k=cell(s,lv);if(k)rows.push({label:s.name+(L==='all'?' ('+lv+')':'')+(k._holdout?'  · holdout':'  · WF✓'),k,category:s.category,s,lv});});});
  } else if(L==="all"){
