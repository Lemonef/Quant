"""
NEW FAMILY: relative-value (cross-sectional). Buy coins that underperformed BTC over a lookback
(they tend to catch up). Different from absolute capitulation (flush) and trend. Check correlation
to trend & flush + blend contribution. 4h, 25 coins, 2021-2026.
"""
...
btc_ret=lambda idx,L: btc.pct_change(L).reindex(idx)

# trend & flush for correlation
...
# relative-value: long coins underperforming BTC by >thr over L bars; hold H bars or convergence
L,THR,H=6,0.15,6
def rv_coin(s):
    c=M[s].close; ex=(c.pct_change(L)-btc.pct_change(L).reindex(c.index)).values
    p=np.zeros(len(c)); held=0
    for i in range(L+1,len(c)):
        if held>0:
            p[i]=1.0
            if ex[i]>0 or held>=H: held=0
            else: held+=1
            continue
        if ex[i]<-THR: p[i]=1.0; held=1
    pos=pd.Series(p,index=c.index); return (pos.shift(1).fillna(0)*c.pct_change()-pos.diff().abs().fillna(0)*0.0006).rename(s)
relval=((1+pd.concat([rv_coin(s) for s in coins],axis=1).fillna(0).mean(axis=1)).resample("1D").prod()-1).reindex(IDX).fillna(0)
...
oo=int(len(IDX)*0.6)
print("correlations:")
print(pd.DataFrame({"trend":trend,"flush":flush,"relval":relval}).corr().round(2).to_string())
def row(t,pr):
    f=met(pr); o=met(pr.iloc[oo:]); print(f"  {t:28s} FULL Sh{f[2]:.2f} DD{f[1]:5.1f}% | OOS C{o[0]:5.1f}% DD{o[1]:5.1f}% Sh{o[2]:.2f}")
print("\nsleeves + blends:")
row("relval standalone", relval)
row("trend70/flush30 (current)", 0.7*trend+0.3*flush)
row("trend60/flush25/relval15", 0.6*trend+0.25*flush+0.15*relval)
row("trend55/flush25/relval20", 0.55*trend+0.25*flush+0.20*relval)
